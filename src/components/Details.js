...
class Details extends Component {
    render() {
        return (
            <ProductConsumer>
                {(value) =>{
                    const {id, company, img, info, price, title, inChart} = value.detailProduct;
            return(
                <div className="container py-5">
                    ...
                    <Link to="/">
                        <ButtonContainer className="mr-3">BACK TO PRODUCT</ButtonContainer>
                    </Link>
                    <ButtonContainer
                         cart
                        disabled={inChart ? true : false}
                        onClick={()=> {
                            value.addToCart(id);
                            value.openModal(id);
